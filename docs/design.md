...
各ステップは `activeFrom` を持ち、その日以降のレビュー母数にだけ含める。新しく追加したステップは初期値として追加日を `activeFrom` にする。これにより、今日追加した「スクワット5回」が昨日以前の達成率を下げない。

入力UIでは「ステップを追加」「分岐を追加」で枠を増やしていく。内部表現としては、以下のようなテキスト記法と同等の構造を保存する。
...
- ステップ別達成率。有効開始日前の日は、そのステップの母数に含めない
- ステップ間通過率が最も低い接続
- 分岐ごとの選択回数と完了回数
